...
export function AccountSignIn({ next }: { next?: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // cfw-amfn: same void-logError pattern as cfw-qeb6
      // ForgotPasswordForm. Don't await — re-enable the form and
      // surface the inline error immediately; Sentry ships via
      // keepalive in the background.
      void logError("AccountSignIn", "login", err);
