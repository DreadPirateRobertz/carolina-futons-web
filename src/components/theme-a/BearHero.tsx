...
export function BearHero() {
  ...
  return (
    ...
    >
      ...
      {/* The bear — center stage, eyes track cursor */}
      <g transform="translate(960 670)">
        <Bear pose="sitting" scale={2.4} eyesTrack={eyes} blink={blink} />
      </g>

      {/* Drifting clouds */}
      <g transform={`translate(${((time * 15) % 2200) - 200} 130)`} opacity="0.85">
        <Cloud scale={1.2} />
      </g>
      <g transform={`translate(${((time * 10 + 600) % 2200) - 200} 200)`} opacity="0.7">
        <Cloud scale={0.9} />
      </g>
      <g transform={`translate(${((time * 8 + 1200) % 2200) - 200} 90)`} opacity="0.6">
        <Cloud scale={0.7} />
      </g>
