...
describe("getCrossSellProducts — success", () => {
  ...
  it("skips .ne('_id', …) when source._id is missing (partial-source callers)", async () => {
    // The reader allows a partial source (e.g. "recommended from this collection"
    // callers) that doesn't have a product _id. In that case we must NOT call
    // .ne() at all — otherwise we'd filter by `undefined` and get 0 results.
    const source = { collectionIds: ["col-futons"] };
    const { chain, calls } = makeChain([]);
    const client = { products: { queryProducts: vi.fn(() => chain) } };
    vi.doMock("@/lib/wix-client", () => ({ getWixClient: () => client }));
    ...
    expect(calls.find((c) => c.method === "ne")).toBeUndefined();
    expect(calls.find((c) => c.method === "hasSome")).toBeDefined();
  });
