...
describe("POST /api/cross-rig — auth", () => {
  it("returns 401 when secret header is missing", async () => {
    ...
    const res = await POST(makeRequest({ ...BASE_BODY, event: "badge_earned" }, { secret: null }));
    expect(res.status).toBe(401);
  });

  it("returns 401 when secret header is wrong", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "badge_earned" }, { secret: "wrong" }));
    expect(res.status).toBe(401);
  });

  it("returns 401 when secret header is empty string", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "badge_earned" }, { secret: "" }));
    expect(res.status).toBe(401);
  });

  it("accepts request with correct secret", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "badge_earned", payload: { badgeId: "b1" } }));
...
describe("POST /api/cross-rig — schema validation", () => {
  ...
  it("returns 400 when memberId is empty string", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, memberId: "", event: "badge_earned" }));
    expect(res.status).toBe(400);
    expect((await res.json()).error).toMatch(/memberId/i);
  });
  ...
  it("returns 400 when sourceRig is missing", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ memberId: "m1", event: "badge_earned", payload: {} }));
    expect(res.status).toBe(400);
  ...
  it("returns 400 when sourceRig is unknown", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "badge_earned", sourceRig: "unknown_rig" }));
    expect(res.status).toBe(400);
    expect((await res.json()).error).toMatch(/sourceRig/i);
  });
  ...
  it("returns 400 when event is unsupported (e.g. streak_extended)", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "streak_extended" }));
    expect(res.status).toBe(400);
  ...
  it("returns 400 when payload is an array (not object)", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "badge_earned", payload: [] }));
    expect(res.status).toBe(400);
  ...
  it("returns 200 when payload field is absent (defaults to {})", async () => {
    ...
    void _omit;
    const res = await POST(makeRequest({ ...bodyWithoutPayload, event: "ar_discovery_completed" }));
    expect(res.status).toBe(200);
  });
});
...
describe("POST /api/cross-rig — ar_discovery_completed", () => {
  ...
  it("returns 200 for ar_discovery_completed with empty payload", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "ar_discovery_completed", payload: {} }));
    expect(res.status).toBe(200);
  });
});
...
describe("POST /api/cross-rig — badge_earned", () => {
  ...
  it("logs badge_earned event (without memberId in log)", async () => {
    const POST = await route();
...
describe("POST /api/cross-rig — tier_changed", () => {
  ...
  it("returns 400 when tier_changed payload lacks newTier", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "tier_changed", payload: {} }));
    expect(res.status).toBe(400);
  ...
  it("returns 400 when tier_changed newTier is not a string", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "tier_changed", payload: { newTier: 3 } }));
    expect(res.status).toBe(400);
    expect((await res.json()).error).toMatch(/newTier/i);
  });
});
...
describe("POST /api/cross-rig — response shape", () => {
  ...
  it("error response always has ok:false and error string", async () => {
    const POST = await route();
    const res = await POST(makeRequest({ ...BASE_BODY, event: "unknown_event" }));
    const body = await res.json();
    expect(body.ok).toBe(false);
