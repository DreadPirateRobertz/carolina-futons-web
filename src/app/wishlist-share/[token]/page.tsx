// cfw-9vs: Public read-only shared wishlist view — /wishlist-share/[token].
//
// Replaces the legacy /wishlist/[token] surface (which now permanent-
// redirects here) so the share URL no longer collides with the
// authenticated /wishlist route in the URL tree.
//
// No auth required. The token is an HMAC-signed memberId; invalid /
// tampered tokens render the not-found state rather than leaking any
// error detail.
...
function SharedWishlistCard({ item }: { item: WishlistItem }) {
  return (
    <li className="overflow-hidden rounded-lg border border-cf-sand bg-white dark:border-cf-ink/30 dark:bg-cf-cream">
      {item.imageUrl ? (
