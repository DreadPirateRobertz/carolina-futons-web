...
async function verifyTurnstile(
  ...
  try {
    ...
    await logError("contact-form", "Turnstile verify failed", err);
...
export async function sendContactForm(
  ...
  if (!hasSecret && process.env.NODE_ENV === "production") {
    await logError(
      "contact-form",
      "TURNSTILE_SECRET_KEY not set in production — blocking submission",
    );
  ...
  try {
    ...
    await logError("contact-form", "fetch to Velo failed", err);
    return transportFailure(req, TRANSPORT_ERROR_GENERIC);
  }
  ...
  try {
    ...
    await logError(
      "contact-form",
      "failed to parse Velo error body",
      parseErr,
    );
  }
  await logError("contact-form", "Velo endpoint rejected submission", undefined, {
    status: res.status,
    veloError,
  });
...
export async function bookAppointment(
  ...
  if (!env) {
    await logError(
      "appointment-form",
      "SMTP env vars missing — cannot send",
    );
    return {
      status: "error",
      errors: {},
  ...
  try {
    ...
    await logError("appointment-form", "sendMail failed", err);
    return {
      status: "error",
      errors: {},
