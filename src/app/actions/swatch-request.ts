...
async function verifyTurnstile(
  ...
  try {
    ...
    await logError("swatch-request", "Turnstile verify failed", err);
...
export async function listSwatchesAction(): Promise<SwatchListResult> {
  try {
    ...
    await logError("swatch-request", "listSwatchesAction failed", err);
...
export async function submitSwatchRequestAction(
  ...
  try {
    ...
    await logError("swatch-request", "fetch to Velo failed", err);
    return transportFailure(contactInfo, selectedIds, TRANSPORT_ERROR_GENERIC);
  }
  ...
  await logError(
    ...
    undefined,
    { status: res.status, veloError: veloErrorForLog },
  );
  return transportFailure(contactInfo, selectedIds, TRANSPORT_ERROR_GENERIC);
}
