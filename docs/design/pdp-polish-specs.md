...
**Date:** 2026-05-05
**Audience:** the polecat that picks up each of the five queued features below.
**Source-of-truth tokens:** `src/app/globals.css` (`--cf-cta`, `--cf-cream`, `--cf-sand`, `--cf-divider`, `--cf-charcoal`, `--cf-error`).
**Existing scaffolding:** `src/components/product/{PdpReviews,BnplWidget,PdpStickyCta,ProductSpinViewer,CustomerVideoReviewGrid}.tsx`.
...
All five sections obey the same global rules:
...
## 5. Customer video reviews — grid + lightbox polish

**Component:** `src/components/product/CustomerVideoReviewGrid.tsx` (already shipped through cf-ou66.3 / cfw-9zp).
**Polish goal:** the grid renders, but the thumbnails read as static images. We want them to telegraph "playable video" at a glance, and the lightbox needs a clearer caption shelf so the customer's words land alongside their footage.

### 5.1 Surface

```
┌──────────────────────────────────────────────────────────────────┐
│  Customer videos                              5 videos · 1:24 avg│  ← header row
│  ─────────────────────────────────────────────────────────────── │
│                                                                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                        │  ← 3-col grid (md+)
│  │          │  │          │  │          │                        │     1-col on mobile
...
│  │  0:42    │  │  1:08    │  │  0:55    │                        │     duration pill bottom-left
│  └──────────┘  └──────────┘  └──────────┘                        │
│  Sarah M.       David K.       Michelle R.                       │  ← attribution under thumbnail
...
- Aspect ratio fixed at **16:9** via `aspect-video`. Reserves space → no CLS while poster loads.
- Poster image: `next/image fill`, `object-cover`, `priority={false}`. Loading state: `bg-cf-sand` placeholder while the network round-trip lands.
- **Play badge** — bottom-right of the thumbnail:
  - 36 × 36 px circle, `bg-cf-cream/95`, `backdrop-blur`, centered triangle glyph (`▶`), `text-cf-cta`.
  - Drop shadow `shadow-[0_2px_6px_rgba(0,0,0,0.18)]` for depth on busy posters.
  - On card hover (md+): scale 1.0 → 1.08, 180 ms `cubic-bezier(0.32, 0.72, 0, 1)`. Reduced-motion: scale stays 1.0; opacity bumps 0.95 → 1 instead.
- **Duration pill** — bottom-left:
  - `inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium tabular-nums bg-cf-charcoal/75 text-white`.
  - Format `M:SS` (e.g. `1:08`). Sourced from the video metadata; if unavailable, hide the pill — never render `0:00`.
- Card hover (md+): the entire thumbnail scales 1.0 → 1.015 + a 1 px `cf-cta` ring grows in. Reduced-motion: ring still appears, scale doesn't.
- Attribution row sits *outside* the 16:9 frame: `mt-2 text-sm font-medium text-cf-charcoal` for the name, `text-xs text-cf-charcoal/70` for the city.

### 5.3 Header row

- Title: `text-lg font-heading font-semibold text-cf-espresso` (matches `PdpReviews`).
- Right-aligned summary: "5 videos · 1:24 avg" — `text-sm text-cf-charcoal/70`. Hide the average if any video is missing duration metadata.
- Section gets a `border-t border-cf-sand/60` separator above (already in the component). Keep.

### 5.4 Lightbox

```
                              ┌─ X ┐                                ← 44×44 close, top-right of viewport
       ┌───────────────────────────────────────────────┐
       │                                               │
       │           [ playing video, 16:9 ]             │            ← max-width 960 px
       │                                               │
       │                                               │
       └───────────────────────────────────────────────┘
       ┌───────────────────────────────────────────────┐
       │  Sarah M. · Asheville, NC · Mar 12 2026       │            ← caption shelf
       │  "Frame is silent — best night's sleep we've  │
       │   had in a year."                              │
       │  Featured product:  Kingston Futon Frame  →   │            ← inline product link, cf-cta
       └───────────────────────────────────────────────┘
                                                                    ↓ backdrop click + ESC dismiss
```

- Backdrop: `bg-cf-charcoal/85 backdrop-blur-sm`, fixed, full-viewport.
- Backdrop click + `Escape` dismiss. On open, focus moves to the close button (`X`); on close, focus returns to the originating thumbnail (already wired via `useRef` pattern in `CustomerVideoReviewGrid.tsx`).
- Crossfade open/close: 180 ms ease-out. Reduced-motion: snap.
- **Caption shelf** — under the player, max-width matches the player. Three rows:
  1. Attribution + date — `text-sm text-cf-charcoal/70`.
  2. Customer quote — `text-base text-cf-charcoal` (italics if Stilgar likes that read; default upright). Render the customer's actual words; do not paraphrase.
  3. Featured product link — only render when `productSlug` is present; format `<a href="/products/{slug}" class="text-cf-cta inline-flex items-center gap-1 text-sm font-medium">Featured product: {productName} →</a>`. Closes the lightbox on click and SPA-navigates.
- Don't autoplay. Player loads paused with the poster + native controls.
- For YouTube iframes: pass `?rel=0&modestbranding=1` to suppress related-video clutter on end-state.

### 5.5 Empty / single-video states

- 0 videos: section returns `null` (already implemented). No empty header, no "no videos yet" copy.
- 1 video: still render the grid — single full-width card on mobile, single column-1 card on md+ (don't promote a single video to a hero treatment; it reads like a featured-content tile and breaks the "social proof" framing).

### 5.6 A11y

- Each thumbnail is a `<button type="button" aria-label="Play video by {customerName}, {duration}">`. Don't rely on the play badge alone.
- Lightbox: `role="dialog" aria-modal="true" aria-labelledby={captionId}` so AT users hear the customer attribution as the dialog title.
- Captions / transcripts: out of scope here, but Stilgar should track whether collected videos ship with WebVTT before this becomes a long-tail problem. Leave a `// TODO(captions)` in the code, not the doc.
...
- Customer video reviews: do we want video transcripts under the player for SEO + a11y? Recommend yes for any video over 30 seconds, but that's a separate ingestion flow — out of scope for the visual polish bead.
